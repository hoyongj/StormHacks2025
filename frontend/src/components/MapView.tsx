...
function MapView({
    ...
    useEffect(() => {
        ...
        async function fetchMapsKey() {
            ...
            try {
                ...
                const payload: { googleMapsApiKey?: string } =
                    await response.json();
                if (cancelled) {
                    return;
                }
                ...
                if (!cancelled) {
                    setMapError(
                        "Live map is unavailable because the Google Maps API key is not configured."
                    );
                }
            } finally {
                if (!cancelled) {
    ...
    useEffect(() => {
        ...
        loader
            ...
            .then((google) => {
                ...
                Promise.all(
                    stops.map((stop) =>
                        geocodeStop(stop, geocoder, cacheRef.current)
                    )
                )
                    .then((results) => {
                        if (!active || !plan) {
                        ...
                        if (!markersRef.current.length) {
                            ...
                            const singlePosition =
                                getMarkerPosition(markersRef.current[0]) ??
                                DEFAULT_CENTER;
        ...
        <div className="map">
            ...
            <div className="map__controls">
                ...
                >
                    <button
                        ...
                        className={
                            mode === "single"
                                ? "map__mode-button map__mode-button--active"
                                : "map__mode-button"
                        }
            ...
            {!plan ? (
                <div className="map__message">
                    Select a plan to preview the route.
                </div>
            ) : null}
            {mapError ? (
    ...
    async function loadMultiLegRoute(
        planId: string,
...
async function geocodeStop(
    stop: Pick<
        PlanStop,
        "label" | "description" | "placeId" | "latitude" | "longitude"
    >,
